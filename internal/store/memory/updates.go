...
func (s *UpdateEventStore) BatchByCursor(_ context.Context, cursors []store.EventCursor) ([]domain.UpdateEvent, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	out := make([]domain.UpdateEvent, 0, len(cursors))
	for _, cursor := range cursors {
		for _, event := range s.events[cursor.UserID] {
			if event.Pts == cursor.Pts {
				out = append(out, cloneUpdateEvent(event))
				break
			}
		}
