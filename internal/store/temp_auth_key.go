...
type TempAuthKeyBindingStore interface {
	...
	// SaveWithState performs the same write and returns the effective protocol
	// Layer tuple established by the store composition. The v2 production
	// wrapper commits identity in PostgreSQL and then merges Layer state in Redis.
