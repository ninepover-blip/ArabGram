...
// canonicalizeStoredPhoneIdentities is the data half of migration 0190. SQL
...
CREATE TEMP TABLE phone_identity_0190 (
...
LIMIT $2`, usersTable.Sanitize()), lastID, phoneIdentityMigrationBatchSize)
		...
		if len(candidates) > 0 {
			...
			if _, err := tx.CopyFrom(ctx,
				pgx.Identifier{"phone_identity_0190"},
	...
	if _, err := tx.Exec(ctx, `
CREATE INDEX phone_identity_0190_canonical_idx
ON phone_identity_0190(canonical_phone, user_id)`); err != nil {
...
FROM phone_identity_0190 AS first
JOIN phone_identity_0190 AS second
...
FROM phone_identity_0190 AS planned
