...
func (a *plainPrivateSendBatchActor) Eligible(messageStore *MessageStore) bool {
	if a == nil || messageStore == nil {
		return false
	}
	_, ok := messageStore.db.(*pgxpool.Pool)
	return ok
}
...
func (a *plainPrivateSendBatchActor) Submit(
	...
	case <-ctx.Done():
		// Once the actor has accepted a task, cancellation only stops the waiter.
		// The bounded worker may still commit; random_id replay is the receipt.
		return domain.SendPrivateTextResult{}, ctx.Err()
	}
}
...
func selectPlainPrivateSendBatch(
	...
	for _, task := range pending {
		...
		if selectedStore != nil && task.store != selectedStore {
			blocked = true
		}
...
func plainPrivateSendTaskUsers(task *plainPrivateSendBatchTask) []int64 {
	...
	if task.req.SenderUserID == task.req.RecipientUserID {
		return []int64{task.req.SenderUserID}
	}
	if task.req.SenderUserID < task.req.RecipientUserID {
		return []int64{task.req.SenderUserID, task.req.RecipientUserID}
	}
	return []int64{task.req.RecipientUserID, task.req.SenderUserID}
}
...
func (a *plainPrivateSendBatchActor) worker() {
	for batch := range a.work {
		...
		var batchErr error
		if len(results) > 0 {
			batchErr = results[0].err
		}
		if len(batch) > 0 && batch[0] != nil && batch[0].store != nil && batch[0].store.batchObserver != nil {
			batch[0].store.batchObserver.PlainPrivateSendBatch(time.Since(started), len(batch), batchErr)
		}
		a.batches.Add(1)
...
func executePlainPrivateSendBatch(tasks []*plainPrivateSendBatchTask) []plainPrivateSendBatchResult {
	...
	batchUsers := make(map[int64]struct{}, len(tasks)*2)
	for _, task := range tasks {
		...
		for _, userID := range plainPrivateSendTaskUsers(task) {
			if _, exists := batchUsers[userID]; exists {
	...
	lockUsers = normalizeAccountWriteUserIDs(lockUsers)
	allocationUsers = normalizeAccountWriteUserIDs(allocationUsers)
	...
	releaseAdmission, err := processAccountWriteAdmission.Acquire(ctx, lockUsers...)
	if err != nil {
	...
	// A batch emits multiple dispatch_outbox INSERT statements. Their statement
	// triggers otherwise acquire shared lane fences in task order, while Egress
	// acquires the corresponding exclusive fences in global stream order when an
	// active lane becomes empty. Pre-acquire the complete producer lock set in
	// that same global order before the first write so the two transactions can
	// never form an A(shared)-B(exclusive) / B(shared)-A(exclusive) cycle.
	...
	if _, err := tx.Exec(ctx, `SELECT set_config('telesrv.defer_dialog_light', 'on', true)`); err != nil {
		return plainPrivateSendBatchErrorResults(results, fmt.Errorf("defer plain private send batch dialog invalidation: %w", err))
	}
	...
	releaseAdmission()
	for i := range tasks {
		if results[i].err == nil && !results[i].result.Duplicate {
			messageStore.publishPrivateDialogInvalidations(results[i].result)
		}
	}
	return results
}
...
func lockDispatchOutboxAppendFences(ctx context.Context, tx pgx.Tx, userIDs []int64) error {
	userIDs = normalizeAccountWriteUserIDs(userIDs)
	if len(userIDs) == 0 {
		return nil
	}
	if _, err := tx.Exec(ctx, `
SELECT pg_advisory_xact_lock_shared(outbox_lane_advisory_key('account_pts', stream_id))
FROM unnest($1::bigint[]) AS requested(stream_id)
ORDER BY stream_id`, userIDs); err != nil {
		return fmt.Errorf("lock plain private send dispatch append fences %v: %w", userIDs, err)
	}
	return nil
}
