...
SELECT bind_status, effective_layer, effective_observation_id
...
`,
		...
		b.ExpiresAt,
		b.EncryptedMessage,
	).Scan(&status, &effectiveLayer, &observationID)
	if err != nil {
	...
	switch status {
	case "ok":
		if effectiveLayer < 0 || observationID < 0 || (observationID > 0 && effectiveLayer == 0) {
			return domain.TempAuthKeyBindingResult{}, fmt.Errorf(
				"bind temporary auth key atomically: invalid result layer=%d observation=%d",
				effectiveLayer, observationID,
			)
		}
		return domain.TempAuthKeyBindingResult{Layer: effectiveLayer, LayerObservationID: observationID}, nil
	case "already_bound":
