...
func (r *Router) publishBootstrapAfterBaseline(ctx context.Context, userID int64) bool {
	...
	return r.publishBootstrapAfterBaselineExplicit(ctx, userID, authKeyID, sessionID)
}

// publishBootstrapAfterBaselineExplicit is the cross-service form used by the
// typed post-response action. CoreExec delivery callbacks run on a fresh gRPC
// context, so the business auth key and physical session must travel in the
// action instead of being recovered from process-local request context.
func (r *Router) publishBootstrapAfterBaselineExplicit(ctx context.Context, userID int64, authKeyID [8]byte, sessionID int64) bool {
	if r.deps.BootstrapUpdates == nil || userID == 0 || authKeyID == ([8]byte{}) || sessionID == 0 {
		return false
	}
