...
// User delivery is committed by verificationPeerVerifier inside the decision
// transaction. The post-commit notifier may only invalidate projections.
func TestNotifyPeerVerifiedUserIsCacheOnly(t *testing.T) {
	const (
	...
	if pushed := sessions.pushedUserIDs(); len(pushed) != 0 {
		t.Fatalf("post-commit verification hook pushed %v", pushed)
	}
}
