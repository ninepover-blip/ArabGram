...
// send photo/video/audio/voice/document messages, every member consumes the
// channel nudge and recovers the messages through getChannelDifference, and
// then every member downloads every resulting file.
//
	...
	for i, record := range records {
		...
		client, err := newClient(endpoint, publicKey, &EncryptedFileStorage{Path: resolveSessionPath(manifestPath, record), Key: key}, clientHooks{
			Update: telegram.UpdateHandlerFunc(func(_ context.Context, updates tg.UpdatesClass) error {
				if pts, ok := groupMediaChannelNudge(updates, group.ChannelID); ok {
					counters.channelNudges.Add(1)
					holder.requestChannelDifference(pts)
				}
				return nil
			}),
...
func uploadGroupMedia(
	...
	for _, upload := range tasks {
		...
		go func() {
			...
			target, err := uploadAndSendGroupMedia(ctx, upload.client.raw.Load(), upload.source, marker, group, cfg.OperationTimeout, fanout, metrics)
			results <- struct {
				target groupMediaTarget
				err    error
