...
// API_BASE is read from VITE_API_BASE at build time (set in .env or vercel.json).
// When empty the panel talks to the same origin — the normal self-hosted layout.
...
export const api = {
  ...
  reviewModerationAppeal: (caseID: number, appealID: number, payload: Record<string, unknown>) =>
	...
	stickerDocumentAnimationURL: (documentID: string) => `${API_BASE}/api/stickers/documents/${encodeURIComponent(documentID)}/animation`,
	gifCatalogDocumentPreviewURL: (documentID: string) => `${API_BASE}/api/gif-catalog/documents/${encodeURIComponent(documentID)}/preview`,
